...
<head>
  ...
  <title><?php print $nombresistema; ?> - Reporte de Créditos</title>
    ...
    <div class="row">
      <div class="col-xs-12">
        <h2 class="page-header">
          <i class="fa fa-list-alt"></i> Reporte de Créditos (<?php print $nomcliente; ?>  -  <?php print $nomestado; ?>)

          <a class="btn btn-success color-palette btn-grad pull-right" target="_blank" href="<?php print $base_url;?>credito/reportecreditoXLS" data-original-title="" title=""><i class="fa fa-file-excel-o fa-1x"></i> Exportar a Excel </a>
        </h2>
      </div>
      <!-- /.col -->
    </div>
        ...
        <table class="table table-striped">
          ...
          <?php 
            ...
            foreach ($credito as $co) {
              $total = $total  + $co->montocredito - $co->abonado;
              
          ?>
            <tr>
              <td><?php @$fec = str_replace('-', '/', @$co->fecha); @$fec = date("d/m/Y", strtotime(@$fec)); print @$fec;  ?></td>
              <td><?php print addslashes(@$co->nom_cliente); ?></td>
              <td><?php print @$co->ident_cliente; ?></td>
              ...
              <td>$ <?php print number_format(@$co->abonoinicial,2,",","."); ?></td>
              <td>$ <?php print number_format(@$co->montointerescredito,2,",","."); ?></td>
              <td>$ <?php print number_format(@$co->montocredito,2,",","."); ?></td>
              <td>$ <?php print number_format(@($co->montocredito - $co->abonado),2,",","."); ?></td>
              <td><?php print addslashes(@$co->telefonos_cliente); ?></td>
              <td><?php print addslashes(@$co->correo_cliente); ?></td>
              <td><?php print addslashes(@$co->direccion_cliente); ?></td>
            </tr>
          <?php 
            }
          ?>
