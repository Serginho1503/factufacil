<?php
/* ------------------------------------------------
  ARCHIVO: venta_retencion.php
  DESCRIPCION: Contiene la vista de retenciones del módulo de venta.
  FECHA DE CREACIÓN: 07/08/2017
 * 
  ------------------------------------------------ */
// Setear el título HTML de la página
$nombresistema = $this->session->userdata("nombresistema");
print "<script>document.title = '$nombresistema - Venta Retenciones'</script>";
...
<script type='text/javascript' language='javascript'>
  ...
  $(document).ready(function () {
    ...
    $("#txt_factura").mask("999-999-999999999");
   ...
   tablecomp=$('#dataTableRet').dataTable({
    ...
    $(document).on('click', '.add_conceptoretencion', function(){
      var subtotalconiva = "<?php if(@$comp != NULL){ print $comp->descsubconiva; } else {print 0;}?>";
      var subtotalsiniva = "<?php if(@$comp != NULL){ print $comp->descsubsiniva; } else {print 0;}?>";
      $.fancybox.open({
        type: "ajax",
        width: 550,
        height: 550,
        ...
        href: "<?php echo base_url('Facturar/add_retencion');?>" 
      });
    });    
    ...
    $(document).on('click', '.ret_upd', function(){
      ...
      $.fancybox.open({
        ...
        href: "<?php echo base_url('Facturar/editar_retencion');?>" 
      });
    });    
    ...
    $(document).on('click', '.ret_del', function(){
      ...
      if (confirm("Desea eliminar el concepto de retencion (" + str + ")?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Facturar/eliminar_retencionrenta",
             data: {id: id},
    ...
    $(document).on('click', '.upd_retencion', function(){
      ...
      if (confirm("Desea guardar la retencion de la venta?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Facturar/guardar_retencion",
             data: {txt_id_comp_ret: txt_id_comp_ret, txt_factura: txt_factura, txt_autorizacion: txt_autorizacion, fecha_ret: fecha_ret,
    ...
    $(document).on('click', '.del_retencion', function(){
      ...
      if (confirm("Desea eliminar la retencion de la venta?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Facturar/eliminar_retencion",
             data: {id: id},
             success: function(json) {
                if (parseInt(json) == 1) {
                   location.replace("<?php print $base_url ?>Facturar/ventas");
    ...
    $(document).on('click', '.btnguardardetalle', function(){ 
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: base_url + "Facturar/guardar_detalleretencion",
...
*/
      ...
      function actualizatotalretenido(){
          ...
          var montototal = parseFloat($("#txtmontototal").text());
          $("#txtmontodif").text((montototal - (retenidorenta + retiva10 + retiva20 + retiva30 + retiva50 + retiva70 + retiva100)).toFixed(2));
      }  
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-registered"></i> Retenciones en Venta
    </h1>
    <ol class="breadcrumb">
      <li><a href="<?php print $base_url ?>inicio"><i class="fa fa-dashboard"></i> Inicio</a></li>
      <li class="active"><a href="<?php print $base_url ?>facturar/ventas">Listado de Ventas</a></li>
      ...
      <div class="col-md-12" >
        <div class="box box-danger" >
          <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-truck"></i> Datos de la Venta </h3> 
              <div class="pull-right"> 
                ...
                <input type="hidden" id="txt_id_comp_ret" name="txt_id_comp_ret" value="<?php if(@$comp != NULL){ print @$comp->id_venta_ret;} else {print 0;} ?>">
              </div>
          </div>
          <div class="box-body">
            <div class="row">
              <div class="col-md-12" style="padding-left: 0px; padding-right: 0px">
                <div class="col-md-4">
                  <label class="col-md-12">Fecha: <span class="text-red"><?php if(@$comp != NULL){ $fec = str_replace('-', '/', $comp->fecharegistro); @$fec = date("d/m/Y", strtotime(@$fec)); print $fec;}?></span></label>
                  <label class="col-md-12">Cliente: <span class="text-red"><?php if(@$comp != NULL){ print $comp->nom_cliente; }?></span></label>
                  <label class="col-md-12">C.I./R.U.C.: <span class="text-red"><?php if(@$comp != NULL){ print $comp->ident_cliente; }?></span></label>
                </div>
                ...
                <div class="col-md-3" style="padding-left: 0px; padding-right: 0px">
                  <label class="col-md-12">Subtotal IVA %12: <span class="text-red"><?php if(@$comp != NULL){ print $comp->descsubconiva; }?></span></label>
                  <label class="col-md-12">Subtotal IVA %0: <span class="text-red"><?php if(@$comp != NULL){ print $comp->descsubsiniva; }?></span></label>
                  <label class="col-md-12">Monto IVA: <span class="text-red"><?php if(@$comp != NULL){ print $comp->montoiva; }?></span></label>
                </div>                  
                <div class="col-md-2" style="padding-left: 0px; padding-right: 0px">
                  <label class="col-md-12">Valor Total: <span id="txtmontototal" class="text-red"><?php if(@$comp != NULL){ print $comp->montototal; }?></span></label>
                  <label class="col-md-12">Retenido: <span id="txtretenido" class="text-red"><?php if(@$comp != NULL){ print $comp->montoretenido; }?></span></label>
                  <label class="col-md-12">Total a Cobrar: <span id="txtmontodif" class="text-red"><?php if(@$comp != NULL){ print number_format($comp->montototal - $comp->montoretenido,2,".",","); }?></span></label>
                </div>                  
              </div>
            </div>
          </div>
      ...
      <div class="row">
        ...
        <div class="col-md-4" style="padding-left: 0px; padding-top: 0px; padding-bottom: 0px;">
         ...
         <div class="col-md-12" style="padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px;">
          ...
          <div class="box box-danger" >
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-registered"></i> Datos de la Retencion </h3> 
            </div>
            <div class="box-body">
              <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="form-group col-md-8">
                      <input type="text" class="form-control validate[required] text-center" id="txt_factura" name="txt_factura" value="<?php if(@$comp != NULL){ print @$comp->nro_retencion;}  ?>">
                    </div>
                    <div class="form-group col-md-4">
            ...
            <div class="col-md-12" style="padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px;">
              <div class="box box-danger">
                <div class="box-header with-border">
                  ...
                  <div class="pull-right"> 
                    <a id="btn_addcptoret" class="btn bg-light-blue-active color-palette add_conceptoretencion" <?php if(@$comp != NULL){ if ((@$comp->descsubsiniva+@$comp->descsubconiva) == @$comp->totalbaseretenido) {print 'disabled'; }}?> href="#" data-original-title="" title=""><i class="fa fa-plus-square"></i> Añadir Concepto </a>
                  </div>
