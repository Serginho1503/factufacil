...
print "<script>document.title = '$nombresistema - Compra Retenciones'</script>";
   ...
   tablecomp=$('#dataTableRet').dataTable({
    ...
    $(document).on('click', '.add_conceptoretencion', function(){
      ...
      $.fancybox.open({
        ...
        href: "<?php echo base_url('Compra/add_retencion');?>" 
      });
    });    
    ...
    $(document).on('click', '.ret_upd', function(){
      ...
      $.fancybox.open({
        ...
        href: "<?php echo base_url('Compra/editar_retencion');?>" 
      });
    });    
    ...
    $(document).on('click', '.ret_del', function(){
      ...
      if (confirm("Desea eliminar el concepto de retencion (" + str + ")?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Compra/eliminar_retencionrenta",
             data: {id: id},
    ...
    $(document).on('click', '.upd_retencion', function(){
      ...
      if (confirm("Desea guardar la retencion de la compra?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Compra/guardar_retencion",
             ...
             success: function(json) {
                if (parseInt(json) == 1) {
                   location.replace("<?php print $base_url ?>compra");
                } else {
    ...
    $(document).on('click', '.del_retencion', function(){
      ...
      if (confirm("Desea eliminar la retencion de la compra?")){
        $.ajax({
             type: "POST",
             dataType: "json",
             url: base_url + "Compra/eliminar_retencion",
             data: {id: id},
             success: function(json) {
                if (parseInt(json) == 1) {
                   location.replace("<?php print $base_url ?>compra");
    ...
    $(document).on('click', '.btnguardardetalle', function(){ 
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: base_url + "Compra/guardar_detalleretencion",
    ...
    $(document).on('change','#cmb_punto', function(){
      ...
      if (punto == tmppunto){
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: base_url + "Compra/sel_nroret_ptoemi",
            data: { punto: punto },
            success: function(json) {
              if (json != null){
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-registered"></i> Retenciones en Compra
    </h1>
    <ol class="breadcrumb">
      <li><a href="<?php print $base_url ?>inicio"><i class="fa fa-dashboard"></i> Inicio</a></li>
      <li class="active"><a href="<?php print $base_url ?>compra">Listado de Compras</a></li>
      ...
      <div class="col-md-12" >
        <div class="box box-danger" >
          <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-truck"></i> Datos de la Compra </h3> 
              <div class="pull-right"> 
                ...
                <input type="hidden" id="txt_id_comp_ret" name="txt_id_comp_ret" value="<?php if(@$comp != NULL){ print @$comp->id_comp_ret;} else {print 0;} ?>">
              </div>
          </div>
          <div class="box-body">
            <div class="row">
              <div class="col-md-12" style="padding-left: 0px; padding-right: 0px">
                <div class="col-md-4">
                  <label class="col-md-12">Fecha: <span class="text-red"><?php if(@$comp != NULL){ $fec = str_replace('-', '/', $comp->fecha); @$fec = date("d/m/Y", strtotime(@$fec)); print @$fec;}?></span></label>
