...
class RetencionSRI extends ComprobanteSRI
{
  ...
  function CrearArchivoXML()
  {
        ...
        $registro = $this->objmodel->datosRetencionCompra($this->id);
        ...
        $detalles = $this->objmodel->detalleRetencionIVA($this->id);
        ...
        $detalles = $this->objmodel->detalleRetencionRenta($this->id);
        foreach($detalles as $detalle){
            $impuesto = $xml->createElement("impuesto");
            $totalImpuesto = $totalConImpuestos->appendChild($impuesto);
      ...
      $divstr.='<tr>';
        ...
        $divstr.='<div class="cuadro_izq" style="float:left; width:350px; ">';
            $divstr.='<div class="cuadro_logo" style="height:140px; padding-top:60px; ">';
              ...
              if ($this->tipo == 2){
                $imagen = $this->objmodel->sel_logoempresaretcompra($this->id);
              }
              else{
                $imagen = $this->objmodel->sel_logoempresaretgasto($this->id);
              }
              if (trim($imagen) != ''){
  ...
  function obtenerMensajeCorreo($objfac){
    $msg = "<!DOCTYPE html>
            ...
            <div style='margin: 0 auto; background-color: white; width:800px; overflow-y:auto;'>
                <h4>Estimado(a) proveedor  $objfac->nom_proveedor </h4>
                <hr>
                <p>
                    <strong>Nos complace informarle que su documento electrónico ha sido generado con el siguiente detalle:</strong>
                    <br/>
                    <br/>
                    <strong>Tipo de documento: RETENCION</strong> 
                    <br/>
                    <strong>Documento electrónico No: </strong>$objfac->cod_establecimiento-$objfac->cod_puntoemision-$objfac->nro_retencion
                    <br/>
                    <br/>
  ...
  function actualiza_enviado($claveacceso){
    $this->objmodel->actualiza_enviadoRetencion($this->id, $claveacceso);
  }

  function actualiza_autorizado($fechaautorizo){
    $this->objmodel->actualiza_autorizadoRetencion($this->id, $fechaautorizo);
  }

  function actualiza_rechazado($claveacceso){
    $this->objmodel->actualiza_rechazadoRetencion($this->id, $claveacceso);
  }

  function sel_claveacceso(){
    return $this->objmodel->sel_claveaccesoRetencion($this->id);
  }

  function datoscomprobante(){
    return $this->objmodel->datosRetencionCompra($this->id);
  }

  function actualiza_correoenviado(){
    $this->objmodel->actualiza_correoenviadoRetCompra($this->id);
  }
